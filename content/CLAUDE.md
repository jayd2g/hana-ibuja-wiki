...
- 분석 작업 문서는 `Consulting/`에 저장 (카테고리별 정리)
...
> - `Consulting/` : 소스들을 분석·종합해서 작성한 작업 문서. 카테고리별로 분류.
...
> **이 규칙은 이 프로젝트의 모든 Cowork 채팅에 적용된다.**

1. **대화 시작 시**: `wiki/index.md`를 먼저 읽어 위키 전체 구조를 파악한다
2. **질문에 답할 때**: 관련 위키 페이지를 먼저 확인하고, 위키 내용을 기반으로 답변한다
3. **위키에 없는 내용**: "위키에 없음, 내 추론임"이라고 명확히 구분해서 말한다
4. **전략 관련 질문**: `Consulting/05_Tobe전략/` 먼저 확인한다
5. **사용자 데이터/인터뷰 관련 질문**: `Consulting/03_사용자/` 및 `wiki/sources/` 확인한다
